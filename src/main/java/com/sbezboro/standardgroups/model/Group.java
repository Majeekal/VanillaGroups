...
public class Group extends PersistedObject implements Comparable<Group> {
	...
	public static final String WAR_AREA = "wararea";
	...
	public Group(FileStorage storage, String name, long established, StandardPlayer leader) {
		...
		this.maxClaims.setValue(VanillaGroups.getPlugin().getGroupStartingLand());
	...
	public void loadProperties() {
		...
		for (Claim claim : claims) {
			if (locationToClaimMap.containsKey(claim.getLocationKey())) {
				VanillaGroups.getPlugin().getLogger().severe("Duplicate claim for " + getName() + " - " + claim.getX() + ", " + claim.getZ());
	...
	public boolean isWarArea() {

		return getName().equals(Group.WAR_AREA);
	}
	...
	public List<StandardPlayer> getPlayers() {
		...
		for (String uuid : memberUuids) {
			StandardPlayer player = VanillaPlugin.getPlugin().getStandardPlayerByUUID(uuid);
			list.add(player);
		}
	...
	public int getWeightedClaimCount() {
		return claims.getList().size() + (VanillaGroups.getPlugin().getSpawnClaimCost() - 1) * spawnClaims.getValue();
	}
	...
	public void enableAutoCommand(String uuid, String[] args) {
		...
		VanillaPlugin.getPlugin().getStandardPlayerByUUID(uuid).sendMessage("Autocommand " +
	...
	public void disableAutoCommand(String uuid) {
		...
		VanillaPlugin.getPlugin().getStandardPlayerByUUID(uuid).sendMessage("Autocommands disabled");
	}
	...
	public Claim claim(StandardPlayer player, Location location) {
		...
		if (VanillaGroups.getPlugin().getGroupManager().isNextToSpawn(location)) {
	...
	public void unclaim(Claim claim) {
		...
		if (VanillaGroups.getPlugin().getGroupManager().isNextToSpawn(claim.getWorld(), claim.getX() << 4, claim.getZ() << 4)) {
			spawnClaims.setValue(spawnClaims.getValue() - 1);
		}
	...
	public void recalculateSpawnClaims() {
		...
		for (Claim claim : getClaims()) {
			if (VanillaGroups.getPlugin().getGroupManager().isNextToSpawn(
	...
	public boolean canLock(Location location) {
		...
		return lockCount == null || lockCount < VanillaGroups.getPlugin().getMaxLocksPerChunk();
	}
	...
	public long getNextGrowth() {
		return getLastGrowth() + VanillaGroups.getPlugin().getGroupLandGrowthDays() * 86400000;
	}
	...
	public void grow() {
		int growthAmount = VanillaGroups.getPlugin().getGroupLandGrowth();
		int maxAmount = VanillaGroups.getPlugin().getGroupLandGrowthLimit();
	...
	public void addPower(double difference) {
		...
		double minAmount = VanillaGroups.getPlugin().getGroupPowerMinValue();
		double maxAmount = Math.min(VanillaGroups.getPlugin().getGroupPowerMaxValue(), getMaxPower());
	...
	public int getLockedBedsCount() {
		...
		for (Lock lock : locks) {
			if (VanillaPlugin.BED_BLOCKS.contains(lock.getLocation().getBlock().getType())) {
				count++;
			}
		}
	...
	public List<Group> getFriendedGroups() {
		...
		for (String uid : getFriendedGroupUids()) {
			...
			if (group == null) {
				VanillaGroups.getPlugin().getLogger().severe("Group " + getName() + " still friends invalid group " + uid);
			} else {
				groups.add(group);
			}
		}
