...
public class GroupManager extends BaseManager {
	...
	private static final HashSet<Material> PROTECTED_BLOCKS = new HashSet<Material>() {{
		...
		addAll(VanillaPlugin.BED_BLOCKS);
		add(Material.CHEST);
		add(Material.BARREL);
		add(Material.OAK_TRAPDOOR);
	...
	public GroupManager(VanillaPlugin plugin, VanillaGroups subPlugin, GroupStorage storage) {
		super(plugin);
	...
	public boolean isGroupsAdmin(StandardPlayer player) {
		return player.hasPermission("vanillagroups.groups.admin");
	}
	...
	private void handleGroupDeletion(Group group, StandardPlayer byPlayer, boolean automatically) {
		...
		if (automatically) {
			VanillaPlugin.broadcast(ChatColor.YELLOW + "The group " + group.getName() + " has been destroyed automatically.");
		} else if (byPlayer == null) {
			VanillaPlugin.broadcast(ChatColor.YELLOW + "A server admin has destroyed the group " + group.getName() + ".");
		} else {
			VanillaPlugin.broadcast(ChatColor.YELLOW + byPlayer.getDisplayName(false) + " has destroyed their group.");
		}
	}
	...
	public List<Lock> getLocksAffectedByBlock(Group group, Location location) {
		...
		if (lock == null) {
			...
			} else if (VanillaPlugin.BED_BLOCKS.contains(targetBlock.getType())) {
	...
	public void createGroup(StandardPlayer player, String groupName) {
		...
		for (String str : VanillaPlugin.getPlugin().getMutedWords()) {
		...
		VanillaPlugin.broadcast(ChatColor.YELLOW + player.getDisplayName(false) + " has created a new group.");
	}
	...
	public boolean hasCommandCooldown(String uuid, boolean displayMessage) {
		...
		if (new Date().getTime() - lastPlayerCommandMap.get(uuid) <= 5000) {
			if (displayMessage) {
				VanillaPlugin.getPlugin().getStandardPlayerByUUID(uuid).sendMessage(ChatColor.AQUA +
