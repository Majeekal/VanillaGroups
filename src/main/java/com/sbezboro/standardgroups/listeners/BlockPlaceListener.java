...
public class BlockPlaceListener extends SubPluginEventListener<VanillaGroups> implements Listener {

	public BlockPlaceListener(VanillaPlugin plugin, VanillaGroups subPlugin) {
		...
		if (victimGroup != null) {
			...
			// Unauthorized. Canceling
				...
				if (victimGroup.isSafeArea()) {
					...
					player.sendMessage(ChatColor.RED + "Cannot place blocks in the war area");
				} else {
					player.sendMessage(ChatColor.RED + "Cannot place blocks in the territory of " + victimGroup.getName());
				}
			}
