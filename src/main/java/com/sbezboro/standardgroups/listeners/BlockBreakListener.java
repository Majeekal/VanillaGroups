...
public class BlockBreakListener extends SubPluginEventListener<VanillaGroups> implements Listener {
	
	public BlockBreakListener(VanillaPlugin plugin, VanillaGroups subPlugin) {
	...
	public void onBlockBreak(BlockBreakEvent event) {
		...
		if (victimGroup != null) {
			...
			// Player not in group, but group has low power, so blocks may be breakable
				...
				if (VanillaPlugin.BED_BLOCKS.contains(event.getBlock().getType())) {
				...
				if (event.getBlock().getType() == Material.ICE && !player.getItemInHand().getEnchantments().containsKey(Enchantment.SILK_TOUCH)) {
					event.setCancelled(true);
